...
var factories = [
	...
	{ 
		'nome': 'logoutSvc',
		'url': '/rest/Usuario/Sair' 
	},
	{ 
		'nome': 'restAmbienteVersaoSvc',
		'url': '/rest/Ambiente/Versao' 
	}
];
...
angular.module('stn-framework-frontend', [])
	...
	.factory('utilsSvc', function (toastr, ngDialog, $http) {
		...
		_obj.enumFrequencia = [{ id: 'D', valor: 'Diária' }, { id: 'S', valor: 'Semanal' }, { id: 'M', valor: 'Mensal - Dia específico' }, {
			id: 'P', valor: 'Mensal - Primeiro dia útil do mês'
		}, { id: 'U', valor: 'Mensal - Último dia útil do mês' }, { id: 'I', valor: 'Manual' }];
		_obj.enumDiaSemana = [
	...
	/**
	 * @ngdoc function
	 * @name modeloApp.controller:ControllergeralCtrl
	 * @description
	 * # ControllergeralCtrl
	 * Controller of the modeloApp
	 */

	.controller('ControllergeralCtrl', function ($rootScope, $scope, logoutSvc, toastr, ngDialog, $location, metaFactory) {
		...
		metaFactory['restAmbienteVersaoSvc'].get(function (data) {
			$scope.versaoAplicacao = data.versaoAplicacao;
			$scope.versaoFramework = data.versaoFramework;
		});


		//Monitora a alteração de perfil. 
		$scope.$watch(function ($rootScope) {
			return $rootScope.usuarioLogado;
		}, function () {
			if (JSON.parse(localStorage.getItem('usuario')) != null) {
				$scope.login = JSON.parse(localStorage.getItem('usuario')).name;
				//exibe o perfil ativo na página	
				metaFactory['restUsuarioPerfil'].get(function (data) {
		...
		$scope.modalAlterarPerfis = function () {
			metaFactory['LoginSvc'].get('', function (data) {
				$scope.perfis = data.roles;
				$scope.perfilEscolhido = $scope.perfil;
				ngDialog.open({
					template: 'views/perfil_modal.html',
					scope: $scope
				});
			});
		}
		//configura o estilo do cabeçalho conforme o ambiente
		var classePagina = '';
		var carregarAmbiente = function () {
			var url = $location.absUrl();
			if (url.indexOf('localhost:9000') > -1) {
				classePagina = 'cabecalho-apex-dev';
				$scope.identificacaoAmbiente = ' - AMBIENTE DESENVOLVIMENTO';
			} else if (url.indexOf('https://homologa.tesouro/') > -1) {
				classePagina = 'cabecalho-apex-homol';
				$scope.identificacaoAmbiente = ' - AMBIENTE HOMOLOGAÇÃO';
			} else {
				classePagina = 'cabecalho-apex';
				$scope.identificacaoAmbiente = '';
			}

		}
		carregarAmbiente();
		$scope.classePagina = function () {
			return classePagina;
		}

		$scope.sair = function () {
			localStorage.perfilSet = undefined;
			toastr.info('Desconectando...');

			logoutSvc.get('', function () {
				localStorage.usuario = JSON.stringify({});
				localStorage.usuarioPerfil = "";
				localStorage.logado = "false";
				localStorage.perfilSet = undefined;
	...
	/**
	 * @ngdoc function
	 * @name modeloApp.controller:ExemplocrudListaCtrl
	 * @description
	 * # ExemplocrudListaCtrl
	 * Controller of the modeloApp
	 */
	/*global objApp */
	.controller('HomeUsuarioCtrl', function ($scope, crudService, utilsSvc) {

		$scope.obj = {};
		$scope.anexosSelecionados = false;
		$scope.URL_SISTEMA = URL_SISTEMA;

		// Configuração dos atributos crudService.
		crudService.scope = $scope;
		crudService.factory = 'restItemProcessadoSvc';
		crudService.templateDetalhar = 'views/artigoconhecimento_detalhe.html';

		// Gera as funções de crud no escopo atual.
		crudService.prepararCrudService($scope);


		//configura funcões compartilhadas	
		utilsSvc.carregarFuncoes($scope);
	...
	/**
	 * @ngdoc function
	 * @name modeloApp.controller:LoginCtrl
	 * @description
	 * # LoginCtrl
	 * Controller of the modeloApp
	 */
	/*global URL_SISTEMA */
	/*global objApp */
	.controller('LoginCtrl', function ($scope, $timeout, $rootScope, toastr, $http, $location, perfilSvc, ngDialog, metaFactory) {
		//variaveis
		var URL_Login = URL_SISTEMA + '/j_security_check';
		$scope.online = true;
		var timer;

		//garante que o usuário não está logado	
		$scope.checarLogado = function () {

			metaFactory['LoginSvc'].get({}, function (data) {
				//conseguiu conectar. atualiza a variável. 
				$scope.online = true;
				...
				if (data.name === undefined || data.name === '') {
					...
					//usuário já autenticado. Redireciona para a tela inicial. 
				...
				function (data) {
					//não foi possível acessar o sistema. Sistema fora do ar. 
					if (data.data.indexOf("Access to the requested resource has been denied") > -1) {
						toastr.warning("O usuário foi autenticado, mas não possui acesso a este sistema.");
						metaFactory['logoutSvc'].get('', function () {
							localStorage.usuario = JSON.stringify({});
							localStorage.usuarioPerfil = "";
							localStorage.logado = "false";
							localStorage.perfilSet = undefined;
			...
			//a cada 10 segundos verifica se o sistema está online
			timer = $timeout($scope.checarLogado, 10000);
		};
		...
		$scope.logar = function () {
			$http({
				method: 'POST',
				url: URL_Login,
				data: 'j_username=' + $scope.username + '&j_password=' + encodeURIComponent($scope.password),
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			}).then(function () {
				//recupera o perfil 
				metaFactory['LoginSvc'].get({}, function (data) {
					if (data.name === undefined || data.name === '') {
						toastr.error('Usu\xE1rio ou senha inv\xE1lida.');
					} else {

						perfilSvc.checarLogin($scope);
						$timeout.cancel(timer);
						toastr.success('Usu\xE1rio Autenticado.');
					}
				}, function () {
					toastr.error('Erro ao recuperar perfil de usu\xE1rio.');
				});
			}, function () {
				$scope.checarLogado();
			});
	...
	/**
	 * @ngdoc function
	 * @name modeloApp.controller:NavbarCtrl
	 * @description
	 * # NavbarCtrl
	 * Controller of the modeloApp
	 */
	...
	.controller('NavbarCtrl', function ($scope, $rootScope, $location) {
		...
		//expoe variaveis para o escopo
		$scope.url = URL_SISTEMA;
